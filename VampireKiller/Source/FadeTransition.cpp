...
{
    is_active = false;
    from = GameState::INITIAL_SCREEN;
    to = GameState::INITIAL_SCREEN;
    scene_from = 0;
    scene_to = 0;
    frames_from = 0;
    frames_to = 0;
...
void FadeTransition::SetScene(int to, int frames_to, const Rectangle& rect)
{
    is_active = true;
...
void FadeTransition::SetScene(int from, int frames_from, int to, int frames_to, const Rectangle& rect)
{
    is_active = true;

    this->scene_from = from;
    this->scene_to = to;
    this->frames_from = frames_from;
    this->frames_to = frames_to;
    frames_counter = 0;
...
void FadeTransition::SetActive(bool active)
{
    is_active = active;
}
...
void FadeTransition::Render()
{
    ...
    if (is_active)
    {
        if (state == FadeState::OUT)
        {
            ...
            Color color = Fade(BLACK, 1/*alpha*/);
